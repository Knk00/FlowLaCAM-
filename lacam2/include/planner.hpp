...
struct HNode {
  ...
  HNode(const Config& _C, HighwayDistTable& D, HNode* _parent, const uint _g,
        const uint _h);
  ~HNode();
};
...
struct Planner {
  ...
  // intermediate/checkpoint parameters
  const bool _intermediate;
  const int _intermediate_freq;
  ...
  Planner(const Instance* _ins, const Deadline* _deadline, std::mt19937* _MT,
          ...
          const std::string& _congestion_path = "data/processed/random-32-32-20/even_11_agent_11_reconstructed.bin",
          const bool _intermediate = false,
          const int _intermediate_freq = 1000);
  ~Planner();
  Solution solve(std::string& additional_info, bool& is_intermediate_solution);
  
  // Getter method to access the HighwayDistTable after solving
  const HighwayDistTable& getHighwayDistTable() const { return HD; }
  HighwayDistTable& getHighwayDistTable() { return HD; }
