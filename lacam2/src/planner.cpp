      ...
      search_tree(std::queue<LNode*>())
{
  ...
  // set priorities
  if (parent == nullptr) {
    // Initialize root node: priority = normalized distance to goal
    for (uint i = 0; i < N; ++i) priorities[i] = (float)D.get(i, C[i]) / N;
  } else {
    // dynamic priorities, akin to PIBT (Priority-based Incremental Best-first Tree search)
    for (size_t i = 0; i < N; ++i) {
      if (D.get(i, C[i]) != 0) {
        // Agent i is NOT at goal: increase priority (more urgent)
        priorities[i] = parent->priorities[i] + 1;
      } else {
        // Agent i IS at goal: reset priority to fractional part only
        // This keeps agents at goal with lower priority
...
// for high-level 
HNode::HNode(const Config& _C, HighwayDistTable& HD, HNode* _parent, const uint _g,
             const uint _h)
    : C(_C),
      parent(_parent),
      neighbor(),
      ...
      search_tree(std::queue<LNode*>())
{
  ...
  if (parent == nullptr) {
    // Initialize root node: priority = normalized distance to goal
    for (uint i = 0; i < N; ++i) priorities[i] = (float)HD.get(i, C[i]) / N;
  } else {
    // dynamic priorities, akin to PIBT (Priority-based Incremental Best-first Tree search)
    for (size_t i = 0; i < N; ++i) {
      if (HD.get(i, C[i]) != 0) {
...
Planner::Planner(const Instance* _ins, const Deadline* _deadline,
                 ...
                 const Objective _objective, const float _restart_rate, const std::string& _congestion_path,
                 const bool _intermediate, const int _intermediate_freq)
    : ins(_ins),
      deadline(_deadline),
      MT(_MT),
      verbose(_verbose),
      ...
      congestion_path(_congestion_path),
      _intermediate(_intermediate),
      _intermediate_freq(_intermediate_freq),
      N(ins->N),
      V_size(ins->G.size()),
      D(DistTable(ins)),
      HD(HighwayDistTable(ins, _congestion_path)),
      loop_cnt(0),
      C_next(N),
...
Solution Planner::solve(std::string& additional_info, bool& is_intermediate_solution)
{
  solver_info(1, "start search");
  
  // Initialize intermediate flag
  is_intermediate_solution = false;

  // setup agents
  for (auto i = 0; i < N; ++i) A[i] = new Agent(i);

  // setup search  HNode* H_goal = nullptr;          // to store goal node
  ...
  auto H_init = new HNode(ins->starts, HD, nullptr, 0, get_h_value(ins->starts));
  OPEN.push(H_init);
  ...
  while (!OPEN.empty() && !is_expired(deadline)) {
    ...
    // Branch-and-bound pruning: discard nodes that cannot lead to better solutions
    if (H_goal != nullptr && H->f >= H_goal->f) {
      // Current node's f-value (g + h) is >= best known solution cost
      // Since h is admissible, any complete path through H will cost >= H->f
      // Therefore, this branch cannot improve our best solution - safe to prune
      OPEN.pop();
      continue;
    }

    // check goal condition  HNode* H_goal = nullptr;          // to store goal node
    ...
    if (H->g >= _intermediate_freq && _intermediate && _intermediate_freq > 0) {
      H_goal = H;
      is_intermediate_solution = true;
      solver_info(1, "timestep checkpoint reached at: ", H->g);
      if (objective == OBJ_NONE) break;
      continue;
    }
    ...
    // Duplicate detection: check if this configuration was already explored
    const auto iter = EXPLORED.find(C_new);
    if (iter != EXPLORED.end()) {
      // CASE 1: Configuration already exists - we found a cycle or alternative path
      
      // Create edge connection between current node and the existing node
      // This allows for path cost updates through the graph structure
      rewrite(H, iter->second, H_goal, OPEN);
      
      // Restart strategy: decide whether to continue from the existing node or restart
      // This helps escape local minima and explore different search directions
      auto H_insert = (MT != nullptr && get_random_float(MT) >= RESTART_RATE)
                          ? iter->second    // Continue from existing node (exploitation)
                          : H_init;         // Restart from beginning (exploration)
      
      // Only add to search queue if it could potentially improve the solution
      if (H_goal == nullptr || H_insert->f < H_goal->f) OPEN.push(H_insert);
    } else {
      // CASE 2: New configuration - create a brand new search node
      
      // Create new HNode with:
      // - C_new: the new configuration (where all agents will be)
      // - D: distance table for heuristics
      // - H: parent node
      // - H->g + edge_cost: actual cost from start to this new node
      // - get_h_value: heuristic estimate from this node to goal
      const auto H_new = new HNode(
          C_new, HD, H, H->g + get_edge_cost(H->C, C_new), get_h_value(C_new));
      
      // Add to explored list to prevent future duplicates
      EXPLORED[H_new->C] = H_new;
      
      // Only add to search queue if it could potentially lead to better solution
...
bool Planner::funcPIBT(Agent* ai)
{
  ...
  std::sort(C_next[i].begin(), C_next[i].begin() + K + 1,
          [&](Vertex* const v, Vertex* const u) {
            return HD.get(i, v) + tie_breakers[v->id] < HD.get(i, u) + tie_breakers[u->id];
            // auto cost_v = HD.get(i, v);
            // auto cost_u = HD.get(i, u);
            // if (cost_v == cost_u) {
            //   return tie_breakers[v->id] < tie_breakers[u->id];
            // }
            // else {
            //   return cost_v < cost_u;
            // }
          });
                               ...
                               Vertex* v_pusher_origin, Vertex* v_puller_origin)
{
  ...
  Vertex* tmp = nullptr;
  while (HD.get(pusher, v_puller) < HD.get(pusher, v_pusher)) {
  ...
  return (HD.get(puller, v_pusher) < HD.get(puller, v_puller)) &&
         (HD.get(pusher, v_pusher) == 0 ||
          HD.get(pusher, v_puller) < HD.get(pusher, v_pusher));
}
