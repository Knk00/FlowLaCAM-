...
MODEL_DIR = "./data/test_huber_loss_stable_stage3_model/*_model.txt"  # Changed to match flat file structure
...
def process_file(model_file, baseline_dir):
    ...
    # if unique_id contains agents less than 10 then skip processing as we are not interested in those files
    # Check if the unique_id ends with any agent count from 1 to 9
    if any(unique_id.endswith(f'_agent_{i}') for i in range(1, 10)):
        return None, None

    baseline_output = baseline_dir + unique_id + ".txt"
    baseline_output = baseline_output.replace('-', '_')  # Adjust for any '-' to '_' changes in filenames
...
if __name__ == "__main__":
    ...
    # Separate model and baseline results, filtering out None values
    model_results_list = []
    baseline_results_list = []
    
    for result in results:
        # Skip if result is None (filtered files) or if either component is None
        if result is None or result == (None, None):
            continue
            
        model_result, baseline_result = result
        
        # Only add if both model and baseline results are valid
        if model_result is not None and baseline_result is not None:
            model_results_list.append(model_result)
            baseline_results_list.append(baseline_result)
        else:
            # Log which files had issues
            if model_result is None and baseline_result is not None:
                print(f"Warning: Model result missing for {baseline_result.get('unique_id', 'unknown')}")
            elif baseline_result is None and model_result is not None:
                print(f"Warning: Baseline result missing for {model_result.get('unique_id', 'unknown')}")
    ...
    combined_df.to_csv("./data/performance/comparison_test_huber_loss_stable_stage3_model__.csv", index=False)
    print("Results saved to ./data/performance/comparison_test_huber_loss_stable_stage3_model__.csv")