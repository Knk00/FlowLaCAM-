"""
Stage 3 Training with Fixed Planner Batch Sampling

Key improvement: Uses a fixed set of training batches for planner evaluation across all epochs
instead of random sampling. This ensures:
1. Consistent gradient updates across epochs
2. Stable convergence metrics 
3. Meaningful learning progress tracking
4. Reproducible training behavior

The same subset of batches will be used for planner evaluation in every epoch,
allowing the model to learn consistently from the same examples.
"""
...
OUTPUT_DIR = "./data/output_stage3_retry_lacam_/"
...
PLANNER_BATCH_PROB = 0.12  # Increase planner frequency in Stage 3
...
if __name__ == "__main__":
    ...
    model_path = "./data/model/model_epoch_10_20251017-232711.pth"
    ...
    best_model_path = os.path.join(MODEL_SAVE_DIR, "best_model_curriculum_stable_huber_loss_stage3.pt")
    ...
    training_metrics = {
        ...
        'lambda_coeff': [], 'stage_weights': [], 'planner_batches_used': []
    }
    ...
    # Fixed training subset for planner evaluation - CRITICAL for stable training!
    # This ensures the same batches are used for planner evaluation across all epochs
    num_planner_batches = max(1, int(len(train_files) * PLANNER_BATCH_PROB))
    train_planner_indices = random.sample(range(len(train_files)), min(num_planner_batches, len(train_files)))
    print(f"🎯 Fixed planner training batches: {len(train_planner_indices)} out of {len(train_files)} total batches")
    print(f"   This ensures consistent evaluation across epochs for stable convergence")
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        if execute_planner:
            print(f"   🎯 Using fixed {len(train_planner_indices)} planner batches for consistent evaluation")
        ...
        for batch_idx, batch_data in enumerate(train_pbar):
            # Use fixed batch indices for planner evaluation instead of random sampling
            # This ensures consistent evaluation across epochs for stable convergence
            run_planner_this_batch = (execute_planner and batch_idx in train_planner_indices)
        ...
        training_metrics['planner_batches_used'].append(epoch_metrics['planner_batches'])
        ...
        planner_batches_used = epoch_metrics['planner_batches']
        ...
        if execute_planner:
            print(f"  🎯 Planner batches used: {planner_batches_used}/{len(train_planner_indices)} (fixed set for consistency)")
